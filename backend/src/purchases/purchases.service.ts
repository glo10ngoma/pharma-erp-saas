...
export class PurchasesService {
  ...
  private async wrap<T>(callback: () => Promise<T>) { try { return await callback(); } catch (error) { if (this.isPurchaseNumberCollision(error)) throw new BadRequestException('PURCHASE_NUMBER_ALREADY_EXISTS'); if (error instanceof Error) { const bad = ['PURCHASE_NOT_DRAFT','SUPPLIER_NOT_IN_TENANT','SITE_NOT_IN_TENANT','CURRENCY_NOT_FOUND','ARTICLE_NOT_IN_TENANT','PURCHASE_HAS_NO_ITEMS','INVALID_PURCHASE_QUANTITY','INVALID_LOT_NUMBER','INVALID_EXPIRY_DATE','INVALID_CONVERSION_FACTOR','INVALID_STOCK_QUANTITY','EXCHANGE_RATE_REQUIRED','INVALID_PAYMENT_AMOUNT','CASH_SESSION_REQUIRED','CASH_SESSION_NOT_OPEN','SITE_NOT_ALLOWED']; if (bad.includes(error.message)) throw new BadRequestException(error.message); if (error.message === 'PURCHASE_NOT_FOUND') throw new NotFoundException('PURCHASE_NOT_FOUND'); if (error.message === 'PERMISSION_DENIED') throw new ForbiddenException('PERMISSION_DENIED'); } throw error; } }
