...
CREATE TABLE stock_movements (
    ...
    movement_type VARCHAR(50) NOT NULL CHECK (
        movement_type IN ('PURCHASE_IN', 'SALE_OUT', 'TRANSFER_IN', 'TRANSFER_OUT', 'ADJUSTMENT_IN', 'ADJUSTMENT_OUT', 'RETURN_IN', 'RETURN_OUT', 'EXPIRED_OUT')
    ),
...
CREATE TABLE IF NOT EXISTS inventory_sessions (
    ...
    status VARCHAR(30) NOT NULL DEFAULT 'DRAFT'
        CHECK (status IN ('DRAFT', 'COUNTING', 'VALIDATED', 'CANCELLED')),
    notes TEXT,
    created_by UUID REFERENCES users(user_id),
