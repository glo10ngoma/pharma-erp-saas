...
export async function persistBootstrapSnapshot(
  ...
  await replaceAll(tx.objectStore(SETTINGS_STORE), [{
    ...
    autoRegisterWorkstation: payload.settings.autoRegisterWorkstation,
    autoBootstrap: payload.settings.autoBootstrap,
    autoSync: payload.settings.autoSync,
    autoOpenCashSession: payload.settings.autoOpenCashSession,
    autoAllocation: payload.settings.autoAllocation,
    allocationTargetQuantity: payload.settings.allocationTargetQuantity,
    allocationLowThreshold: payload.settings.allocationLowThreshold,
    snapshotFreshnessPolicy: payload.settings.snapshotFreshnessPolicy,
...
export async function applyPosChanges(
  ...
  if (changesPayload.changes.settings.length > 0) {
    ...
    await replaceAll(settingsStore, [{
      ...
      autoRegisterWorkstation: currentSettings?.autoRegisterWorkstation ?? true,
      autoBootstrap: currentSettings?.autoBootstrap ?? true,
      autoSync: currentSettings?.autoSync ?? true,
      autoOpenCashSession: currentSettings?.autoOpenCashSession ?? true,
      autoAllocation: currentSettings?.autoAllocation ?? true,
      allocationTargetQuantity: currentSettings?.allocationTargetQuantity ?? 12,
      allocationLowThreshold: currentSettings?.allocationLowThreshold ?? 3,
      snapshotFreshnessPolicy: currentSettings?.snapshotFreshnessPolicy ?? 'WARN_ONLY',
